...
To grab all detailed program history data from server

```{r}

# Grab Detailed Program History data

PH <- list.files(path = "M:/DATA & REPORTS/Clinical & Client Services/Clinical Data/Program Histories/Detailed Program History and Client Data",  # Identify all CSV files
...
To grab all contact data from server

```{r}

# Grab Contact Data

CD <- list.files(path = "M:/DATA & REPORTS/Clinical & Client Services/Clinical Data/Time Entry",  # Identify all CSV files
                       pattern = "*.csv", full.names = TRUE) %>% 
  map_df(~read_csv(., col_types = cols(.default = "c")))
